...
export function DualSidedIDCard({ person, cardId, viewMode = 'preview' }: DualSidedIDCardProps) {
  const generateQRCode = (text: string) => {
    return `https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(text)}`;
  };

  const CardFront = () => (
    <div className="w-[340px] h-[215px] bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 rounded-xl p-6 text-white relative overflow-hidden shadow-xl border border-slate-700/50">
      {/* Background Pattern */}
      ...
      {/* Security Badges */}
      <div className="absolute top-3 left-3 flex items-center gap-1 bg-emerald-600/90 px-2 py-1 rounded-full text-xs font-medium">
        <Shield className="h-3 w-3" />
        <span>SECURE</span>
      </div>

      <div className="absolute top-3 right-3 flex items-center gap-1 bg-blue-600/90 px-2 py-1 rounded-full text-xs font-medium">
      ...
      <div className="text-center mb-4 pt-6">
        ...
        <div className="w-16 h-0.5 bg-gradient-to-r from-blue-400 to-emerald-400 mx-auto mt-2"></div>
      </div>
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-sm">{person.nrc}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-blue-200 font-medium">DOB:</span>
              <span className="text-sm">{person.dateOfBirth}</span>
            </div>
          </div>
          
          <div className="flex items-center gap-2 mt-2">
            <div className="px-2 py-1 bg-emerald-500/80 rounded text-xs font-semibold flex items-center gap-1">
              <CheckCircle className="h-3 w-3" />
              CITIZEN
            </div>
            <div className="px-2 py-1 bg-blue-500/80 rounded text-xs font-semibold flex items-center gap-1">
  ...
  const CardBack = () => (
    <div className="w-[340px] h-[215px] bg-gradient-to-br from-slate-900 via-slate-700 to-slate-800 rounded-xl p-6 text-white relative overflow-hidden shadow-xl border border-slate-600/50">
      {/* Background Pattern */}
      ...
      <div className="text-center mb-4">
        <h4 className="text-sm font-bold tracking-wider text-blue-200">REPUBLIC OF THE UNION OF MYANMAR</h4>
        <div className="w-20 h-0.5 bg-gradient-to-r from-emerald-400 to-blue-400 mx-auto mt-2"></div>
      </div>
      ...
      <div className="flex items-start gap-4">
        <div className="flex flex-col items-center">
          ...
          <p className="text-xs text-blue-200 font-medium mt-2">VERIFY</p>
        </div>

        <div className="flex-1 space-y-3">
          {/* Contact Information */}
          <div className="space-y-1">
            <h5 className="text-sm font-bold text-emerald-300 flex items-center gap-1">
              <Building2 className="h-4 w-4" />
              ISSUING AUTHORITY
            </h5>
            <p className="text-sm text-blue-100">Ministry of Immigration</p>
            <p className="text-sm text-blue-100">& Population</p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-blue-100">+95-1-234-5678</p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-blue-100">info@mip.gov.mm</p>
          </div>
        </div>
      </div>
  ...
  if (viewMode === 'preview') {
    return (
      <div id={cardId} className="flex justify-center">
        <div className="flex flex-col lg:flex-row gap-6 items-center justify-center">
          <div className="text-center">
            <h4 className="text-sm font-medium text-muted-foreground mb-3">FRONT SIDE</h4>
            <CardFront />
          </div>
          <div className="text-center">
            <h4 className="text-sm font-medium text-muted-foreground mb-3">BACK SIDE</h4>
            <CardBack />
          </div>
        </div>
      </div>
  ...
  return (
    <div id={cardId} className="w-full max-w-4xl mx-auto bg-white p-8">
      <div className="grid grid-cols-2 gap-8 min-h-[297mm]">
        {/* Front Side with cutting guides */}
        <div className="space-y-4">
          ...
          <div className="relative flex justify-center">
            ...
            <div className="w-[85.6mm] h-[54mm] scale-75 origin-center">
              <CardFront />
            </div>
          </div>
        </div>

        {/* Back Side with cutting guides */}
        <div className="space-y-4">
